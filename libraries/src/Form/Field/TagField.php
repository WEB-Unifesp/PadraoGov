...
//****************************************** trecho modificado
		...
		$query3 = $db->getQuery(true);
			 $query3->select('id , fieldparams')
			 ->from('#__fields');		
			 $db->setQuery($query3);
			 $field = $db->loadAssoc();
 
			 $listFieldParams = json_decode($field['fieldparams'] , true);
 
			 foreach($listFieldParams['options'] as $fieldList){
				 if($fieldList['value'] == $tag){
					 $tagName = $fieldList['name'];
				 }
			 }
